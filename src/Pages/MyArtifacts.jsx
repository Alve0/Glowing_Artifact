...
function MyArtifacts() {
  ...
  useEffect(() => {
    const fetchArtifacts = async () => {
      try {
        const response = await fetch(`${url}/my-artifacts`, {
          method: "GET",
          headers: {
            "user-uid": user?.uid || "",
  ...
  const handleDelete = async (id) => {
    try {
      ...
      const data = await response.json();
      alert(data.message || "Artifact deleted successfully");
      setArtifacts(artifacts.filter((artifact) => artifact._id !== id));
      setFilteredArtifacts(
        filteredArtifacts.filter((artifact) => artifact._id !== id)
      );
    } catch (err) {
          ...
          filteredArtifacts.map((artifact) => (
            ...
            >
              ...
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 1.05 }}
                className="btn w-full bg-red-600 mt-2 text-white"
                onClick={() => handleDelete(artifact._id)}
              >
                Delete
              </motion.button>
              <Link to={`/udpate-artifact/${artifact._id}`}>
                <motion.button
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 1.05 }}
                  className="btn w-full text-white mt-2  bg-blue-600"
                >
                  Update
                </motion.button>
              </Link>
